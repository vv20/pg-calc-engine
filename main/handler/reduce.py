...
def handler():
    ...
    for evaluation in evaluation_data.to_dict('records'):
        ...
        counter = 0
        while True:
            next_result: DataFrame = read_store(DataType.PARTITION_RESULT, evaluation_name + '.' + str(counter))
            if next_result.empty:
                break
            result = concat([result, next_result]).sort_values(by=['result'], ascending=False)[:results_size]
            counter += 1
